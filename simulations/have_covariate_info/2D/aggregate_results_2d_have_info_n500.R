#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage results: 2D WITH COVARIATE INFORMATION, n=500
# Z0 = rnorm, B contains signal
...
output_dir <- "outputs_2d_have_info_n500"
...
for (m in methods) {
  vals <- vertex_rates[[m]]  # Coverage rates across 100 reps for each vertex
...
for (est in rownames(sse_all)) {
  cat(sprintf("  %-8s  Mean=%.4f  SD=%.4f\n", est, mean(sse_all[est,]), sd(sse_all[est,])))
}
...
saveRDS(aggregated, file.path(output_dir, "aggregated_2d_have_info_n500.rds"))
cat(sprintf("Aggregated results saved to: %s/aggregated_2d_have_info_n500.rds\n", output_dir))
...
write.csv(summary_df, file.path(output_dir, "summary_2d_have_info_n500.csv"), row.names = FALSE)
cat(sprintf("Summary saved to: %s/summary_2d_have_info_n500.csv\n\n", output_dir))
...
p1 <- ggplot(cov_long, aes(x = Method, y = Coverage, fill = Estimator)) +
  ...
  labs(
    title = sprintf("Coverage Rates: 2D WITH Covariate Info (n=%d, %d reps)", n, n_reps),
...
pdf(file.path(output_dir, "coverage_boxplot_2d_have_info_n500.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
p2 <- ggplot(vertex_df, aes(x = Vertex, y = Coverage, color = Method)) +
  ...
  labs(
    title   = sprintf("Vertex-wise Coverage: 2D WITH Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = rnorm, B contains signal | Dashed line: 95% nominal",
    x = "Vertex Index", y = "Coverage Rate (%)", color = "Method"
  ) +
  theme_minimal(base_size = 13)

pdf(file.path(output_dir, "vertex_coverage_2d_have_info_n500.pdf"), width = 12, height = 6)
print(p2)
dev.off()
...
p3 <- ggplot(cov_long, aes(x = Precision, y = Coverage, fill = Precision)) +
  ...
  labs(
    title   = sprintf("TRUE vs PLUGIN Coverage by Estimator: 2D WITH Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = rnorm, B contains signal",
    x = NULL, y = "Coverage Rate (%)"
  ) +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none")

pdf(file.path(output_dir, "true_vs_plugin_2d_have_info_n500.pdf"), width = 10, height = 5)
print(p3)
dev.off()

cat("Plots saved:\n")
cat(sprintf("  - %s/coverage_boxplot_2d_have_info_n500.pdf\n", output_dir))
cat(sprintf("  - %s/vertex_coverage_2d_have_info_n500.pdf\n", output_dir))
cat(sprintf("  - %s/true_vs_plugin_2d_have_info_n500.pdf\n", output_dir))
cat("\n============================================================================\n")
