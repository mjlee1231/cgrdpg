#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage across 100 replications (n=1000)
# Computes coverage rate for EACH vertex across all reps

cat("============================================================================\n")
cat("  AGGREGATING VERTEX-WISE COVERAGE RESULTS\n")
cat("  2D SPREAD - 100 Replications (n=1000, p_cov=500)\n")
cat("============================================================================\n\n")

# Find all result files in the vertex_wise_results_n1000 directory
result_dir <- "vertex_wise_results_n1000"
...
result_files <- list.files(path = result_dir,
                           pattern = "^vertex_wise_n1000_rep[0-9]{3}\\.rds$",
                           full.names = TRUE)

if (length(result_files) == 0) {
  stop(sprintf("No result files found in %s! Expected: vertex_wise_n1000_rep001.rds through rep100.rds", result_dir))
}
...
# Show some example vertices (spread across 1000)
cat("--- Example Vertices ---\n")
example_vertices <- c(1, 100, 250, 500, 750, 900, 1000)
...
vertex_wise_results <- list(
  ...
  # Vertex-wise coverage (length n = 1000)
...
output_file <- file.path(result_dir, "vertex_wise_coverage_aggregated_n1000.rds")
...
csv_file <- file.path(result_dir, "vertex_wise_coverage_results_n1000.csv")
...
cat("       main='Vertex-wise Coverage Across 100 Reps (n=1000)')\n")
