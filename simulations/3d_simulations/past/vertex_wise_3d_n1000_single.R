#!/usr/bin/env Rscript
# Vertex-wise Coverage for 3D GRDPG with S = diag(1,1,-1): n=1000, 100 reps
# 3D helix latent curve with signature matrix having one negative eigenvalue
...
if (length(args) == 0) {
  stop("Usage: Rscript vertex_wise_3d_n1000_single.R <rep_number>")
}
...
cat("============================================================================\n")
cat("  VERTEX-WISE COVERAGE for 3D GRDPG: n=1000\n")
cat(sprintf("  Running REPLICATION %d/100\n", rep_id))
cat("  Signature matrix: S = diag(1, 1, -1)\n")
cat("============================================================================\n\n")
...
# Generate 3D helix latent positions
# Best design from curve optimization: r=0.175, h=0.040, centers=(0.530, 0.530, 0.315)
# Edge probability range: [0.281, 0.749], width=0.468
cat("Generating 3D helix latent positions (n=1000)...\n")
theta <- pi * (0:(n-1)) / (n - 1)
X0 <- matrix(0, n, 3)
X0[, 1] <- 0.175 * cos(theta) + 0.530
X0[, 2] <- 0.175 * sin(theta) + 0.530
X0[, 3] <- 0.040 * theta / pi + 0.315

# Signature matrix with one negative eigenvalue
S <- diag(c(1, 1, -1))
Y0 <- X0 %*% S
...
cat(sprintf("Edge Probability Width: %.4f\n", max(P) - min(P)))
...
# Fit model using PARALLEL version if available
cat(sprintf("Fitting 3D GRDPG model (maxit=%d, tol=%.3f)...\n", maxit, tol))
fit_start <- Sys.time()

# Try parallel version first, fall back to serial if needed
fit <- tryCatch({
  fit_grdpg_cov_parallel(A, B, d = d, p = 2, q = 1,  # p=2, q=1 for S=diag(1,1,-1)
                         maxit = maxit, tol = tol, tau = tau,
                         ncores = ncores)
}, error = function(e) {
  cat("Parallel fitting not available, using serial version...\n")
  fit_grdpg_cov(A, B, d = d, p = 2, q = 1,
                maxit = maxit, tol = tol, tau = tau)
})
...
# Compute coverage for ALL vertices
cat(sprintf("Computing elliptical coverage for ALL %d vertices...\n", n))
