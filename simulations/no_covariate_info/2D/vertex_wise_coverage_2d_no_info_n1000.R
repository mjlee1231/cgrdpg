...
if (length(args) == 0) stop("Usage: Rscript vertex_wise_coverage_no_info_n1000.R <rep_number>")
...
cat("============================================================================\n")
cat("  Vertex-wise Coverage: NO COVARIATE INFORMATION (n=1000)\n")
...
cat(sprintf("Using %d cores for cgrdpg parallel fitting\n\n", ncores))
  ...
  # 1. Latent positions (same as optimized sims)
  theta <- pi * (1:n) / (n - 1)
  X0 <- matrix(0, n, d)
