...
# Across n=1000, 2000, 3000

cat("============================================================================\n")
cat("  COMPREHENSIVE COMPARISON: ASE/OSE vs FISHER-SCORING\n")
cat("  Comparing coverage rates across sample sizes (n=1000, 2000, 3000)\n")
cat("============================================================================\n\n")
...
# For n=3000, check which file exists
...
extract_coverage <- function(result_obj, method = "fisher") {
  if (method == "ase_ose") {
    ...
    # Fisher-scoring has different structure
    if (!is.null(result_obj$vertex_coverage_mean)) {
      # Format 1: vertex_coverage_mean and vertex_coverage_plugin_mean
      list(
        fisher_true = result_obj$vertex_coverage_mean,
        fisher_plugin = result_obj$vertex_coverage_plugin_mean
      )
    } else if (!is.null(result_obj$coverage_true)) {
      # Format 2: coverage_true/coverage_plugin lists
      list(
        fisher_true = result_obj$coverage_true$mean,
        fisher_plugin = result_obj$coverage_plugin$mean
      )
    } else {
      # Try to find in vertex_coverage sub-object
      list(
...
comparison_df <- data.frame(
  n = c(1000, 2000, 3000),
  ASE_TRUE = c(cov_ase_1000$ase_true, cov_ase_2000$ase_true, cov_ase_3000$ase_true),
  ASE_PLUGIN = c(cov_ase_1000$ase_plugin, cov_ase_2000$ase_plugin, cov_ase_3000$ase_plugin),
  OSE_TRUE = c(cov_ase_1000$ose_true, cov_ase_2000$ose_true, cov_ase_3000$ose_true),
  OSE_PLUGIN = c(cov_ase_1000$ose_plugin, cov_ase_2000$ose_plugin, cov_ase_3000$ose_plugin),
  FISHER_TRUE = c(cov_fisher_1000$fisher_true, cov_fisher_2000$fisher_true, cov_fisher_3000$fisher_true),
  FISHER_PLUGIN = c(cov_fisher_1000$fisher_plugin, cov_fisher_2000$fisher_plugin, cov_fisher_3000$fisher_plugin)
)
...
comparison_results <- list(
  ...
  sse_comparison = data.frame(
    n = c(1000, 2000, 3000),
    ase_sse = c(sse_1000$ase_mean, sse_2000$ase_mean, sse_3000$ase_mean),
    ose_sse = c(sse_1000$ose_mean, sse_2000$ose_mean, sse_3000$ose_mean),
    ose_improvement_pct = c(sse_1000$improvement, sse_2000$improvement, sse_3000$improvement)
  ),
  raw_data = list(
...
saveRDS(comparison_results, "comparison_ase_ose_vs_fisher.rds")
write.csv(comparison_df, "comparison_ase_ose_vs_fisher.csv", row.names = FALSE)

cat("Comparison results saved to:\n")
cat("  - comparison_ase_ose_vs_fisher.rds\n")
cat("  - comparison_ase_ose_vs_fisher.csv\n")
