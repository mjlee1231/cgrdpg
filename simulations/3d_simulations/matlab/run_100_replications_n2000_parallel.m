% Run 100 replications of 3D CGRDPG simulation with fminunc - n=2000 PARALLEL VERSION
% Uses MATLAB parfor for parallel execution on HPC

clear; clc;

%% Parameters (matching R simulations)
n = 2000;  % number of nodes (CHANGED TO 2000)
...
fprintf('========================================\n');
fprintf('100 Replications: 3D CGRDPG with fminunc (PARALLEL, n=2000)\n');
fprintf('========================================\n');
...
parfor rep = 1:n_reps
    ...
    rng_state = RandStream('mt19937ar', 'Seed', rep + 2000);  % Different seed offset
...
%% Summary statistics
fprintf('========================================\n');
fprintf('Summary Statistics (n=2000)\n');
fprintf('========================================\n\n');
...
% Save summary
save('results/fminunc_100reps_n2000_parallel_summary.mat', 'summary');
fprintf('Summary saved to: results/fminunc_100reps_n2000_parallel_summary.mat\n');

% Save detailed results
save('results/fminunc_100reps_n2000_parallel_detailed.mat', 'results_all', '-v7.3');
fprintf('Detailed results saved to: results/fminunc_100reps_n2000_parallel_detailed.mat\n');

% Save individual replications
if ~exist('results/replications_n2000_parallel', 'dir')
    mkdir('results/replications_n2000_parallel');
end

fprintf('Saving individual replications...');
for rep = 1:n_reps
    rep_data = results_all{rep};
    save(sprintf('results/replications_n2000_parallel/rep_%03d.mat', rep), 'rep_data');
end
fprintf(' Done!\n\n');
