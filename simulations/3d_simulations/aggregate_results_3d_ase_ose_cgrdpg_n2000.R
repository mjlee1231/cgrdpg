#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage results: 3D GRDPG, ASE vs OSE vs cgrdpg, n=2000
...
cat("============================================================================\n")
cat("  AGGREGATING: 3D GRDPG ASE vs OSE vs cgrdpg, n=2000\n")
cat("  Methods: cgrdpg-TRUE, cgrdpg-PLUGIN, ASE-TRUE, ASE-PLUGIN, OSE-TRUE, OSE-PLUGIN\n")
cat("  S = diag(1,1,-1) (indefinite signature)\n")
cat("============================================================================\n\n")

results_dir <- "results_3d_ase_ose_cgrdpg_n2000"
...
saveRDS(aggregated, file.path(output_dir, "aggregated_3d_ase_ose_n2000.rds"))
cat(sprintf("Aggregated results saved to: %s/aggregated_3d_ase_ose_n2000.rds\n", output_dir))
...
write.csv(summary_df, file.path(output_dir, "summary_3d_ase_ose_n2000.csv"), row.names = FALSE)
cat(sprintf("Summary saved to: %s/summary_3d_ase_ose_n2000.csv\n\n", output_dir))
...
p1 <- ggplot(cov_long, aes(x = Method, y = Coverage, fill = Estimator)) +
  ...
  labs(
    ...
    subtitle = "S = diag(1,1,-1), p_cov=1000 | Dashed line: 95% nominal",
    x = NULL, y = "Coverage Rate (%)"
  ) +
  theme_minimal(base_size = 13) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

pdf(file.path(output_dir, "coverage_boxplot_3d_ase_ose_n2000.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
pdf(file.path(output_dir, "vertex_coverage_3d_ase_ose_n2000.pdf"), width = 12, height = 6)
print(p2)
dev.off()
...
pdf(file.path(output_dir, "true_vs_plugin_3d_ase_ose_n2000.pdf"), width = 10, height = 5)
print(p3)
dev.off()

cat("Plots saved:\n")
cat(sprintf("  - %s/coverage_boxplot_3d_ase_ose_n2000.pdf\n", output_dir))
cat(sprintf("  - %s/vertex_coverage_3d_ase_ose_n2000.pdf\n", output_dir))
cat(sprintf("  - %s/true_vs_plugin_3d_ase_ose_n2000.pdf\n", output_dir))
cat("\n============================================================================\n")
