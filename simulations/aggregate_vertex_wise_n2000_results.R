#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage across 100 replications (n=2000, PARALLEL)
# Computes coverage rate for EACH vertex across all reps

cat("============================================================================\n")
cat("  AGGREGATING VERTEX-WISE COVERAGE RESULTS\n")
cat("  2D SPREAD - 100 Replications (n=2000, p_cov=1000, PARALLEL)\n")
cat("============================================================================\n\n")

# Find all result files
result_dir <- "vertex_wise_results_n2000_parallel"
...
result_files <- list.files(path = result_dir,
                           pattern = "^vertex_wise_n2000_rep[0-9]{3}\\.rds$",
                           full.names = TRUE)
...
rep_coverage_times <- numeric(n_reps)
...
for (j in 1:n_reps) {
  ...
  rep_coverage_times[j] <- all_results[[j]]$coverage_time
...
# Compute vertex-wise coverage rate
...
# Show example vertices (spread across 2000)
cat("--- Example Vertices ---\n")
example_vertices <- c(1, 200, 500, 1000, 1500, 1800, 2000)
...
cat(sprintf("Fit Time:          Mean=%.1fs, SD=%.1fs (with parallel optimization)\n",
            mean(rep_fit_times), sd(rep_fit_times)))
cat(sprintf("Coverage Time:     Mean=%.1fs, SD=%.1fs\n",
            mean(rep_coverage_times), sd(rep_coverage_times)))
...
vertex_wise_results <- list(
  ...
  # Vertex-wise coverage (length n = 2000)
  ...
  rep_coverage_times = rep_coverage_times,
...
output_file <- file.path(result_dir, "vertex_wise_coverage_aggregated_n2000.rds")
...
csv_file <- file.path(result_dir, "vertex_wise_coverage_results_n2000.csv")
...
cat("       main='Vertex-wise Coverage Across 100 Reps (n=2000)')\n")
