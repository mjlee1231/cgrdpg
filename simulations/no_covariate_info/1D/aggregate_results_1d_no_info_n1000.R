...
p1 <- ggplot(cov_long, aes(x = Method, y = Coverage, fill = Estimator)) +
  ...
  labs(
    title = sprintf("Coverage Rates: 1D No Covariate Info (n=%d, %d reps)", n, n_reps),
...
p2 <- ggplot(vertex_df, aes(x = Vertex, y = Coverage, color = Method)) +
  ...
  labs(
    title   = sprintf("Vertex-wise Coverage: 1D No Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = 0, B = pure noise | Dashed line: 95% nominal",
...
p3 <- ggplot(cov_long, aes(x = Precision, y = Coverage, fill = Precision)) +
  ...
  labs(
    title   = sprintf("TRUE vs PLUGIN Coverage by Estimator: 1D No Covariate Info (n=%d, %d reps)", n, n_reps),
