#!/usr/bin/env Rscript
# Vertex-wise Coverage for 3D GRDPG with S = diag(1,1,-1): n=500, 100 reps
...
if (length(args) == 0) {
  stop("Usage: Rscript vertex_wise_3d_n500_single.R <rep_number>")
}
...
cat("============================================================================\n")
cat("  VERTEX-WISE COVERAGE for 3D GRDPG: n=500\n")
...
cat("Generating 3D helix latent positions (n=500)...\n")
theta <- pi * (0:(n-1)) / (n - 1)
X0 <- matrix(0, n, 3)
