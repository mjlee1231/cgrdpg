function coverage_3d_single_rep_n2000(rep_id)
% COVERAGE_3D_SINGLE_REP_N2000 Compute vertex-wise coverage for 3D GRDPG with n=2000
%
% Same as coverage_3d_single_rep but with n=2000, p_cov=1000
% Tests: cgrdpg-TRUE/PLUGIN, ASE-TRUE/PLUGIN, OSE-TRUE/PLUGIN
%
% Inputs:
...
% Parameters (INCREASED sample size)
n = 2000;
p_cov = 1000;
d = 3;
...
% Output directory
output_dir = 'results_matlab_3d_coverage_n2000';
if ~exist(output_dir, 'dir')
    mkdir(output_dir);
end

fprintf('============================================================================\n');
fprintf('  ASE/OSE/cgrdpg Vertex-wise Coverage: 3D GRDPG, n=%d, p_cov=%d\n', n, p_cov);
fprintf('  Replication %d/100\n', rep_id);
fprintf('  Methods: cgrdpg-TRUE/PLUGIN, ASE-TRUE/PLUGIN, OSE-TRUE/PLUGIN\n');
fprintf('  S = diag([1, 1, -1])\n');
fprintf('============================================================================\n\n');
...
% Same design as n=1000 for fair comparison
t = (1:n)' / n;
X0 = [0.42*t + 0.46, ...
...
% Use fminunc with surrogate (safer, proven to converge)
...
% Use estimated signature S_estimated for PLUGIN method
