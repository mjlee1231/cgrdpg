...
cat("============================================================================\n")
cat("  LASTFM ASIA SOCIAL NETWORK ANALYSIS WITH GRDPG\n")
...
compute_fisher_precision <- function(i, X, tau) {
  ...
  # In practice, you'd use fit_fisher$Z
...
# Performance metrics: ARI, NMI, Silhouette score, k-means clustering accuracy
...
evaluate_clustering <- function(X, labels, method_name) {
  ...
  # Normalized Mutual Information would require additional package
  # For now, we'll use ARI as the main metric

  # Classification accuracy (best permutation match)
  # This is a rough approximation
  confusion <- table(Predicted = km$cluster, True = labels)
  # Find best permutation (greedy matching)
  best_match <- 0
  for (perm in 1:k) {
    correct <- sum(diag(confusion[, order(labels)[1:k]]))
    best_match <- max(best_match, correct)
  }
  accuracy <- best_match / length(labels)
...
saveRDS(results, "lastfm_grdpg_analysis_results.rds")
cat("\n\nResults saved to: lastfm_grdpg_analysis_results.rds\n")
