#!/usr/bin/env Rscript
# Aggregate n=5000 vertex-wise coverage results (PARALLEL + RCPP)
# Reads individual .rds files and computes summary statistics

cat("============================================================================\n")
cat("  AGGREGATING n=5000 VERTEX-WISE COVERAGE RESULTS\n")
cat("  Parallel + Rcpp optimization\n")
cat("============================================================================\n\n")

results_dir <- "vertex_wise_results_n5000_rcpp"
...
result_files <- list.files(results_dir, pattern = "vertex_wise_n5000_rep\\d{3}\\.rds", full.names = TRUE)
...
cat("\n============================================================================\n")
cat("  SUMMARY STATISTICS (n=5000, replications=%d)\n", n_files)
cat("============================================================================\n\n")
...
cat(sprintf("  Fit time:      Mean=%.1fs (%.1f min), SD=%.1fs\n",
            mean(fit_times), mean(fit_times)/60, sd(fit_times)))
cat(sprintf("  Coverage time: Mean=%.1fs (%.1f min), SD=%.1fs\n",
            mean(coverage_times), mean(coverage_times)/60, sd(coverage_times)))
...
cat(sprintf("  TRUE:   Mean=%.2f, Max=%d (out of 5000 vertices)\n", mean(n_na_true), max(n_na_true)))
cat(sprintf("  PLUGIN: Mean=%.2f, Max=%d (out of 5000 vertices)\n\n", mean(n_na_plugin), max(n_na_plugin)))
...
cat(sprintf("Total time for %d reps (Parallel + Rcpp): ~%.1f hours\n", n_files, avg_time_per_rep_mins * n_files / 60))
cat(sprintf("Total time for %d reps (Serial):          ~%.1f hours\n", n_files, estimated_serial_time_mins * n_files / 60))
cat(sprintf("Time saved:                                ~%.1f hours (%.1f days)\n\n",
            (estimated_serial_time_mins - avg_time_per_rep_mins) * n_files / 60,
            (estimated_serial_time_mins - avg_time_per_rep_mins) * n_files / 60 / 24))
...
cat("Comparing with smaller n simulations:\n")
cat("  n=3000 → n=5000 scaling factor: 5000/3000 = 1.67x\n")
cat("  Expected time scaling (theoretical): ~2.78x (due to O(n*(n+p)) coverage)\n")
cat(sprintf("  Actual time scaling: %.2fx\n\n", avg_time_per_rep_mins / 45))  # Assuming n=3000 was ~45 min
...
saveRDS(summary_stats, "summary_vertex_wise_n5000_rcpp.rds")
cat("Aggregated results saved to: summary_vertex_wise_n5000_rcpp.rds\n")
...
write.csv(summary_df, "summary_vertex_wise_n5000_rcpp.csv", row.names = FALSE)
cat("Summary table saved to: summary_vertex_wise_n5000_rcpp.csv\n")
