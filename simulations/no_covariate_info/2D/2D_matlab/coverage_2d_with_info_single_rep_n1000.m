function coverage_2d_with_info_single_rep_n1000(rep_id)
% Vertex-wise Coverage: cgrdpg vs ASE vs OSE WITH COVARIATE INFORMATION
% Scenario: Z0 ~ N(0, I), B = Z0 * X0' + noise (signal present)
% This tests cgrdpg performance when covariates are informative
% n=1000, p_cov=500, d=2 (RDPG)

if nargin < 1
    error('Usage: coverage_2d_with_info_single_rep_n1000(rep_id)');
end
...
output_dir = 'results_2d_with_info_n1000';
if ~exist(output_dir, 'dir')
    mkdir(output_dir);
end

fprintf('============================================================================\n');
fprintf('  Vertex-wise Coverage: WITH COVARIATE INFORMATION (n=%d)\n', n);
fprintf('  Replication %d/100\n', rep_id);
fprintf('  Z0 ~ N(0, I), B = Z0*X0'' + noise\n');
...
% 2. WITH COVARIATE INFORMATION: Z0 ~ N(0, I)
Z0 = randn(p_cov, d);  % Random signal matrix
...
% B contains SIGNAL + NOISE (signal from Z0 * X0')
...
for i = 1:n
    ...
    % OSE (for RDPG: pass same matrix twice since unsigned=signed)
    results.ose_true(i) = check_coverage(...
        X_ose(i,:) - X0(i,:), ...
        compute_prec_ose(i, X0, X0, eps_clip), ...
        chi2_crit, 1);

    results.ose_plugin(i) = check_coverage(...
        X_ose(i,:) - X0(i,:), ...
        compute_prec_ose(i, X_ose, X_ose, eps_clip), ...
        chi2_crit, 1);
end
