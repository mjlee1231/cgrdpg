#!/usr/bin/env Rscript
# Vertex-wise Coverage with PARALLEL + RCPP optimization: n=5000, 100 reps
...
# Expected time per rep: ~90-120 minutes (vs ~540 min with serial)
...
if (length(args) == 0) {
  stop("Usage: Rscript vertex_wise_coverage_n5000_rcpp_100reps.R <rep_number>")
}
...
cat("============================================================================\n")
cat("  VERTEX-WISE COVERAGE with PARALLEL + RCPP: n=5000\n")
...
# Generate data
cat("Generating 2D SPREAD latent positions (n=5000)...\n")
i_vals <- 1:n
theta <- pi * i_vals / (n - 1)
X0 <- matrix(0, n, 2)
...
cat(sprintf("Model fit completed: Converged=%s, Iterations=%d, Time=%.1f sec (%.1f min)\n",
            fit$converged, fit$iters, fit_time, fit_time/60))
...
# Compute coverage for ALL 5000 vertices using RCPP if available
cat("Computing elliptical coverage for ALL 5000 vertices...\n")
...
for (i in 1:n) {
  if (i %% 1000 == 0) cat(sprintf("  Vertex %d/%d (%.1f%%)\n", i, n, 100*i/n))
...
cat(sprintf("Coverage computation completed in %.1f seconds (%.1f min)\n",
            coverage_time, coverage_time/60))
...
# Save results
output_dir <- "vertex_wise_results_n5000_rcpp"
...
output_file <- file.path(output_dir, sprintf("vertex_wise_n5000_rep%03d.rds", rep_id))
saveRDS(results, output_file)

cat("\n============================================================================\n")
cat(sprintf("  REPLICATION %d COMPLETED\n", rep_id))
cat(sprintf("  Total time: %.2f minutes (Fit: %.1f min, Coverage: %.1f min)\n",
            rep_time, fit_time/60, coverage_time/60))
