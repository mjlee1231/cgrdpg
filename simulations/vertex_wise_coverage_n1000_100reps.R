#!/usr/bin/env Rscript
# Vertex-wise Coverage Diagnostic: 2D SPREAD with Oracle Procrustes (n=1000)
# Computes coverage for ALL 1000 vertices - saves per-vertex indicators
...
if (length(args) == 0) {
  stop("Usage: Rscript vertex_wise_coverage_n1000_100reps.R <rep_number>")
}
...
cat("============================================================================\n")
cat("  VERTEX-WISE COVERAGE: 2D SPREAD - Oracle Procrustes (n=1000)\n")
cat(sprintf("  Running REPLICATION %d/100\n", rep_id))
cat("  Computing coverage for ALL 1000 vertices\n")
cat("============================================================================\n\n")

# Fixed parameters (n=1000, p_cov=500)
n <- 1000
p_cov <- 500
d <- 2
maxit <- 30
...
# Compute coverage for ALL 1000 vertices using ELLIPTICAL confidence intervals
cat("Computing elliptical coverage for ALL 1000 vertices...\n")
...
# Using "_n1000" to distinguish from n=500 simulations
output_dir <- "vertex_wise_results_n1000"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Save results with per-vertex indicators for ALL 1000 vertices
results <- list(
  rep = rep_id,
  n = n,
  p_cov = p_cov,
  d = d,
  tau = tau,
  in_ellipse_true = in_ellipse_true,      # Length 1000: TRUE/FALSE/NA for each vertex
  in_ellipse_plugin = in_ellipse_plugin,  # Length 1000: TRUE/FALSE/NA for each vertex
  SSE = SSE,
  fit_time = fit_time,
