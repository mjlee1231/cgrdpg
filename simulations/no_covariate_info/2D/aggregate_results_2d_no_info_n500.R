#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage results: 2D NO COVARIATE INFORMATION, n=500
# Z0 = 0, B = pure noise
...
output_dir <- "outputs_2d_no_info_n500"
...
saveRDS(aggregated, file.path(output_dir, "aggregated_2d_no_info_n500.rds"))
cat(sprintf("Aggregated results saved to: %s/aggregated_2d_no_info_n500.rds\n", output_dir))
...
write.csv(summary_df, file.path(output_dir, "summary_2d_no_info_n500.csv"), row.names = FALSE)
cat(sprintf("Summary saved to: %s/summary_2d_no_info_n500.csv\n\n", output_dir))
...
p1 <- ggplot(cov_long, aes(x = Method, y = Coverage, fill = Estimator)) +
  ...
  labs(
    title = sprintf("Coverage Rates: 2D No Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = 0, B = pure noise | Dashed line: 95% nominal",
    x = NULL, y = "Coverage Rate (%)"
  ) +
  theme_minimal(base_size = 13) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

pdf(file.path(output_dir, "coverage_boxplot_2d_no_info_n500.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
p2 <- ggplot(vertex_df, aes(x = Vertex, y = Coverage, color = Method)) +
  ...
  labs(
    title   = sprintf("Vertex-wise Coverage: 2D No Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = 0, B = pure noise | Dashed line: 95% nominal",
    x = "Vertex Index", y = "Coverage Rate (%)", color = "Method"
  ) +
  theme_minimal(base_size = 13)

pdf(file.path(output_dir, "vertex_coverage_2d_no_info_n500.pdf"), width = 12, height = 6)
print(p2)
dev.off()
...
p3 <- ggplot(cov_long, aes(x = Precision, y = Coverage, fill = Precision)) +
  ...
  labs(
    title   = sprintf("TRUE vs PLUGIN Coverage by Estimator: 2D No Covariate Info (n=%d, %d reps)", n, n_reps),
    subtitle = "Z0 = 0, B = pure noise",
    x = NULL, y = "Coverage Rate (%)"
  ) +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none")

pdf(file.path(output_dir, "true_vs_plugin_2d_no_info_n500.pdf"), width = 10, height = 5)
print(p3)
dev.off()

cat("Plots saved:\n")
cat(sprintf("  - %s/coverage_boxplot_2d_no_info_n500.pdf\n", output_dir))
cat(sprintf("  - %s/vertex_coverage_2d_no_info_n500.pdf\n", output_dir))
cat(sprintf("  - %s/true_vs_plugin_2d_no_info_n500.pdf\n", output_dir))
cat("\n============================================================================\n")
