#!/usr/bin/env Rscript
# Aggregate vertex-wise coverage results: 2D WITH COVARIATE INFORMATION, n=1000
...
output_dir <- "outputs_2d_have_info_n1000"
...
saveRDS(aggregated, file.path(output_dir, "aggregated_2d_have_info_n1000.rds"))
cat(sprintf("Aggregated results saved to: %s/aggregated_2d_have_info_n1000.rds\n", output_dir))
...
write.csv(summary_df, file.path(output_dir, "summary_2d_have_info_n1000.csv"), row.names = FALSE)
cat(sprintf("Summary saved to: %s/summary_2d_have_info_n1000.csv\n\n", output_dir))
...
pdf(file.path(output_dir, "coverage_boxplot_2d_have_info_n1000.pdf"), width = 10, height = 6)
print(p1)
dev.off()
...
pdf(file.path(output_dir, "vertex_coverage_2d_have_info_n1000.pdf"), width = 12, height = 6)
print(p2)
dev.off()
...
pdf(file.path(output_dir, "true_vs_plugin_2d_have_info_n1000.pdf"), width = 10, height = 5)
print(p3)
dev.off()

cat("Plots saved:\n")
cat(sprintf("  - %s/coverage_boxplot_2d_have_info_n1000.pdf\n", output_dir))
cat(sprintf("  - %s/vertex_coverage_2d_have_info_n1000.pdf\n", output_dir))
cat(sprintf("  - %s/true_vs_plugin_2d_have_info_n1000.pdf\n", output_dir))
cat("\n============================================================================\n")
